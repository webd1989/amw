@if($records->count()>0)
 ...
 <tr>
  <td>{{$sr}}</td>
  <td>{{$row->title}}</td>
  <td>{{getDetails('categories',$row->category,'title')}}</td>
  <td>
  	 @if($row->image != '')                      
      @if(file_exists(public_path().'/admin/images/blogs/'.$row->image))
        <img src="{{URL::asset('public/admin/images/blogs')}}/{!! $row->image !!}" style="max-width:100px;height: auto;">
      @endif
    @endif
  </td>
  <td>@php        
  ...
  <td>
      ...
      <div class="dropdown-menu"> 
      	<a class="dropdown-item" style="cursor:pointer" href="{{url('panel/edit-blog',base64_encode($row->id))}}"><i class="icon-base ti tabler-pencil me-1"></i> Edit</a> 
