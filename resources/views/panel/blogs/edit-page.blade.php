                ...
                <div class="tab-content p-0">
                    <div class="tab-pane fade active show" id="form-tabs-personal" role="tabpanel">
                      <div class="row g-2">
                            ...
                            <select id="category" name="category" value="index,nofollow" class="form-select">
                              ...
                              @if(isset($categories) && $categories->count()>0)
                                @foreach($categories as $key => $category)
                                    <option {{$record->category == $key?'selected':''}} value="{{$key}}">{{$category}}</option>
                                @endforeach
                              @endif
                            </select>
                        </div>
                        ...
                        <input type="text" id="added_by" name="added_by" value="{{$record->added_by}}" class="form-control"/>
                      </div>
                      <div class="col-md-4">
                        <label class="form-label" for="title">Author ID</label>
                        <input type="text" id="author_id" name="author_id" class="form-control" value="{{$record->author_id}}" />
                      </div>
                      <div class="col-md-4">
                        <label class="form-label" for="title">Date</label>
                        <input type="date" id="blog_date" name="blog_date" value="{{$record->blog_date}}" class="form-control"/>
                      </div>
                        <div class="col-md-4">
                        ...
                        @if($record->image != '')                      
                          @if(file_exists(public_path().'/admin/images/blogs/'.$record->image))
                          <div class="col-md-2 pt-3">
                            <img src="{{URL::asset('public/admin/images/blogs')}}/{!! $record->image !!}" style="max-width:100px;height: auto;"> 
                          </div>
                          @endif
                        @endif
                        <div class="col-md-4">
                        <label class="form-label" for="title">Image Alt Text</label>
                        <input type="text" id="image_alt" name="image_alt" class="form-control" value="{{$record->image_alt}}" />
                      </div>
                      </div>
                    </div>
