@if($records->count()>0)
 ...
 <tr>
  ...
  <td>@php        
      ...
      @endphp
      <a style="cursor:pointer" onclick="changeStatus('college_categories','{!!$row->id!!}');" id="status_{{$row->id}}" class="badge {{$class}} me-1">{{$label}}</a>
  ...
  <td>
      ...
      <div class="dropdown-menu"> 
      	<a class="dropdown-item" style="cursor:pointer" href="{{url('panel/edit-college-category',base64_encode($row->id))}}"><i class="icon-base ti tabler-pencil me-1"></i> Edit</a> 
      	@if(Session::get('admin_type') == 'Admin') 
			<a class="dropdown-item" onclick="deleteData('college_categories','{{ $row->id }}');" href="javascript:void(0);"><i class="icon-base ti tabler-trash me-1"></i> Delete</a>
        @endif
      </div>
    </div>
  </td>
