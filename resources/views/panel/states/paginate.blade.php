...
<tr>
  ...
  <td>{{$row->id}}</td>
  <td>{{$row->state}}</td>
  <td>{{getDetails('countries',$row->country_id,'title')}}</td>  
  <td> @php
  ...
  <td>
  	<div class="dropdown">
        ...
        <a class="dropdown-item" style="cursor:pointer" href="{{url('panel/edit-state',base64_encode($row->id))}}"><i class="icon-base ti tabler-pencil me-1"></i> Edit</a> 
        @if(Session::get('admin_type') == 'Admin')
        <?php /*?><a class="dropdown-item" onclick="deleteData('states','{{ $row->id }}');" href="javascript:void(0);"><i class="icon-base ti tabler-trash me-1"></i> Delete</a><?php */?>
        @endif
      	</div>
    </div>
  </td>
