...
<div class="container-xxl flex-grow-1 container-p-y">
  <div class="row">
    <div class="col">
      <h6 class="mt-6">Edit College Sub Category</h6>
      <div class="row">
              ...
              <div class="card-body">
                <div class="tab-content p-0">
                  <div class="row g-2">
                    ...
                    <div class="col-md-12">
                    	@php
                        	$statel = explode(',',$record->state_id);                            
                        @endphp
                        <label class="form-label" for="title">State</label>
                        <ul class="list-group state-scroll">
                        @if(isset($stateList) && $stateList->count()>0)
                            @foreach($stateList as $key => $state)
                          <li class="list-group-item">
                            <input class="form-check-input me-1" name="state_id[]" {{(in_array($key,$statel)?'checked':'')}} type="checkbox" value="{{$key}}" id="{{$key}}">
          ...
          <div class="card mb-6">
            <div class="card-body">
              <div class="pt-6">
                ...
                <button type="reset" onclick="window.location.href='{{route('admin.college-sub-categories')}}'" class="btn btn-label-secondary" >Cancel</button>
              </div>
            </div>
          </div>
        </div>
...
var addUrl = "{{url('/panel/edit-college-sub-category/')}}/{{$record->id}}";
$(document).ready(function(){
